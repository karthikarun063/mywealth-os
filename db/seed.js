...
const monthOffset = (n) => { const d = new Date(); d.setMonth(d.getMonth()+n); return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`; };
...
async function seed() {
  ...
  try {
    ...
    for (const [name,cls,qty,pv,cv,notes] of assets) {
      await client.query(
        `INSERT INTO assets(name,asset_class,quantity,purchase_value,current_value,notes) VALUES($1,$2,$3,$4,$5,$6)`,
    ...
    for (const [name,type,amt,rate,emi,lender] of liabs) {
      await client.query(
        `INSERT INTO liabilities(name,liability_type,outstanding_amount,interest_rate,emi,lender) VALUES($1,$2,$3,$4,$5,$6)`,
    ...
    for (const [type,cat,amt,date,rec] of txns) {
      await client.query(
        `INSERT INTO transactions(type,category,amount,date,recurring) VALUES($1,$2,$3,$4,$5)`,
    ...
    const budgetData = [
      ['rent',28000],['groceries',12000],['transport',5000],['utilities',4000],
      ['entertainment',3500],['medical',3000],['shopping',8000],['investments',20000],
      ['food',6000],['dining',4000],
    ];
    for (const [cat,amt] of budgetData) {
      // Insert for current month
      await client.query(
        `INSERT INTO budgets(user_id,category,monthly_budget,month,year)
         VALUES($1,$2,$3,$4,$5) ON CONFLICT(user_id,category,month,year) DO NOTHING`,
        ['default', cat, amt, curMonth, curYear]
      );
      // Insert for previous month (for history)
      await client.query(
        `INSERT INTO budgets(user_id,category,monthly_budget,month,year)
         VALUES($1,$2,$3,$4,$5) ON CONFLICT(user_id,category,month,year) DO NOTHING`,
    ...
    for (const [name,type,tgt,cur_,sip,date,ret] of goals) {
      await client.query(
        `INSERT INTO goals(goal_name,goal_type,target_amount,current_amount,monthly_contribution,target_date,expected_return) VALUES($1,$2,$3,$4,$5,$6,$7)`,
        [name,type,tgt,cur_,sip,date,ret]
      );
    }
    ...
    for (let i=11; i>=0; i--) {
      ...
      await client.query(
        `INSERT INTO snapshots(month,total_assets,total_liabilities,net_worth,savings_rate) VALUES($1,$2,$3,$4,$5) ON CONFLICT(month) DO NOTHING`,
        [mo, a, l, a-l, sr]
      );
    }
